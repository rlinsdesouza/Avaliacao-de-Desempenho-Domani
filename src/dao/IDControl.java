...
public class IDControl {
	...
	public static <T> void atualizaTreeMapLegado(ObjectContainer man, Class c){
		manager = man;
//		EventRegistry eventRegistry = EventRegistryFactory.forObjectContainer(manager);
		int maiorid;
		...
		//localizar e incrementar o ultimo id das classes
		
		Query q = manager.query();
		q.constrain(c);
		List<IDInterface> resultados = q.execute();
		maiorid=0;
		if (!resultados.isEmpty()) {
			for (IDInterface item : resultados) {
				if(item.getId()>maiorid)
					maiorid=item.getId();
			}
		}
		ids.replace(resultados.get(0).getClass().getName(), maiorid);
